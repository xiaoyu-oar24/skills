...
7. **目录级扫描与批量拆解**：
   - 提示词：`帮我扫描 aidocs 目录，找出过长且混杂多个主题的文档并批量拆解`
   - 预期结果：激活 `doc-craftsman` 扫描模式——Glob 枚举 `aidocs/**/*.md`（排除 `.archive/` 与 `INDEX.md`）并逐文件判定，产出三分诊诊断清单（巨石混杂 / 冗余流水账移交 lean-docs / 归类问题移交 docs-layout-quadrant）与拆解计划；经用户确认后**强制落盘执行**原子拆解与三级索引联动（两阶段执行协议），不得停留在建议层面。
...
7. **含"扫描"关键词但意图为治理收敛**：
   - 提示词：`扫描 aidocs 目录，把过期的 plan 归档，再精简啰嗦的设计文档`
   - 预期结果：跳过 `doc-craftsman`，归档与内容精炼由 `lean-docs` 执行（本技能扫描模式只识别多主题混杂并拆解，冗余流水账仅在诊断清单中标注移交）。

8. **自然语言业务开发**：
   - 提示词：`帮我实现用户模块新增接口的业务逻辑`
   - 预期结果：跳过本技能，按项目常规开发流程处理，不自动激活 `xy-feat`；“建议使用”不等同于用户显式调用。

## 显式工作流内的调用豁免

1. 用户已显式启动 `/xy-feat`，阶段 6 调用本技能拆分超阈值接口契约 → 接受该阶段调用，不因“进行中的业务开发”SKIP 而拒绝。
2. 用户仅要求“新建用户模块的设计文档” → 本技能编写内容，档位与布局遵循 `docs-layout-quadrant`，不自动激活 `xy-feat`。
3. `xy-feat v6` 阶段 6 调用拆解，具体文件清单已获授权 → 复用该授权完成拆解与索引；新出现的删除/批量移动超出原范围时仅确认新增范围。完成后返回新路径，分支集成留给 xy-feat 阶段 7。
