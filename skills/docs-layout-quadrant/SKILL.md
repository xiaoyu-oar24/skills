---
name: docs-layout-quadrant
description: "强制文档四象限布局（specs / plan / tracking / guide），规整 docs/ 目录结构，支持与 xy-feat 功能开发流联动。核心原则：领域主体命名（禁用日期前缀）、象限生命周期管理、知识索引导航。TRIGGER when: 开始新功能、编写设计文档或进度报告、整理混乱的 docs/ 目录、xy-feat 开发工作流的指南产出阶段（阶段 5）。SKIP: 项目根目录的 README.md 或 CLAUDE.md 等全局配置文件。"
version: "2.0.0"
author: xiaoyu
---

# docs-layout-quadrant

> **核心思想**：文档是知识图谱，不是 Git Commit 历史。文档应随代码演进而演进，而非每次修改都新建文件。

## 🎯 触发条件

- **TRIGGER when**:
  - 开始规划新功能，需要写设计文档、执行计划、进度跟踪或使用指南时
  - 用户询问"文档放哪里"或"docs/ 目录怎么组织"
  - 整理混乱的 `docs/` 目录，将散落的文档归类到四象限结构中
  - 检查文档命名是否符合规范（发现日期前缀时强制重构）
  - 当作为 `xy-feat` 协同开发工作流的第 5 阶段（指南产出与文档合规整理）被调用时
- **SKIP**: 项目根目录的 `README.md`、`CLAUDE.md` 等全局配置文件

## ⚙️ 依赖环境

- 项目已存在 `docs/` 目录
- Git 版本控制（用于追踪文件移动历史）

## 📖 核心工作流

### 第 1 步：命名规范校验（关键前置）

扫描 `docs/` 下所有 `.md` 文件，**强制执行命名规范**：

**领域主体命名法** — 文件名仅描述模块/领域，**禁止任何日期/时间戳前缀**：
- `<Bad>` `2026-07-15-网关代理转发-design.md` — 一次性产物，AI 无法判断是否为最终状态
- `<Good>` `gateway-proxy-design.md` 或 `网关代理转发.md` — 随代码演进的活文档

发现违规文件时，**自动重命名**（使用 `git mv`），并修复所有引用该文件的内部链接。

### 第 2 步：四象限归类

根据文档类型分配到对应子目录：

| 象限 | 路径 | 定位 | 生命周期 |
| :--- | :--- | :--- | :--- |
| 设计规范 | `docs/specs/` | 需求分析与架构决策（ADR），系统的"宪法"，记录 Why | **长周期** |
| 执行计划 | `docs/plan/` | 实施计划与任务拆解，开发前的 Draft | **短周期** — Feature 交付后标记 `[DONE]` 或废弃 |
| 进度跟踪 | `docs/tracking/` | 任务 Checklist | **超短周期** — 任务验收后归档到 `.archive/` 或删除 |
| 使用指南 | `docs/guide/` | 面向开发者的 How-to 操作手册 | **长周期** |

### 第 3 步：象限生命周期管理

对已有文档执行过期清理：

1. **plan/** — 扫描已完成功能的 plan 文档：
   - 已交付 → 文件名追加 `-done` 后缀，或移入 `docs/plan/.archive/`
   - AI 不应再将已完成的 plan 作为系统现状参考源
2. **tracking/** — 扫描已验收的追踪文档：
   - 任务全部完成 → 移入 `docs/tracking/.archive/` 或直接删除
   - **代码库不该存储执行状态**，Git 历史已足够
3. **specs/ & guide/** — 作为长周期文档，保持原位，但检查内容是否过时

### 第 4 步：建立知识索引

在 `docs/` 根目录维护 `INDEX.md`（若不存在则创建），作为 AI 和人类的知识入口：

```markdown
# 📚 项目知识索引

> 本文件是 docs/ 目录的导航入口，按业务领域组织。新增文档时必须同步更新此索引。

## [领域名称]
| 文档 | 象限 | 文件 |
| :--- | :--- | :--- |
| 网关代理转发设计 | specs | [gateway-proxy-design.md](specs/gateway-proxy-design.md) |
| 网关使用指南 | guide | [gateway-proxy-guide.md](guide/gateway-proxy-guide.md) |
```

每次新增/删除/重命名文档后，**必须同步更新** `docs/INDEX.md`。

### 第 5 步：移动文件与修复链接

1. 使用 `git mv <旧路径> <新路径>` 移动文件（保留 Git 历史）
2. 使用 Grep 搜索文档中引用了旧路径的链接，用 Edit 逐个修正为新的相对路径
3. 所有文档间链接必须使用相对路径（如 `../specs/xxx.md`），**禁止**使用绝对路径

### 第 6 步：验证

使用 Grep 搜索：
- 旧路径残留 → 确认所有引用已更新
- 日期前缀残留（`20[0-9]{2}-[0-9]{2}-[0-9]{2}-*.md`）→ 确认已全部重命名
- INDEX.md 中的链接 → 确认全部可访问

## ⛔ 行为限制与护栏

- **🚫 禁止日期前缀**：文件名不得以日期开头。发现时必须重命名并修复引用。文档是知识图谱，不是流水账。
- **必须使用相对路径**：文档间互相引用时，必须使用相对路径（`./` 或 `../`），禁止使用任何绝对路径（如 `/Users/xxx/` 开头或完整磁盘路径）
- **禁止散落文档**：`docs/` 根目录下只能保留 `INDEX.md` 等入口文件，所有功能性文档必须归入四个象限子目录
- **保护入口文件**：移动和重命名文档时，确保项目根目录的 `README.md` 和 `CLAUDE.md` 不受影响
- **保留 Git 历史**：移动文件时使用 `git mv` 而非普通 `mv`，保留版本历史
- **强制索引同步**：任何文档增删改后，必须同步更新 `docs/INDEX.md`，保持导航网最新

## 📝 模板与范例

### <Bad> 反模式

```markdown
# ❌ 时间戳命名 + 绝对路径 + 散落文档

混乱的文档组织（docs/ 根目录散落文件）：
- /docs/2026-07-15-网关代理转发-design.md
- /docs/2026-07-20-网关代理转发-v2.md   ← 每次修改新建文件
- /docs/跟进度第三期报告.md

绝对路径链接：
[详细设计](/Users/xiaoyu/Documents/code/docs/2026-07-15-网关代理转发-design.md)

问题：
1. AI 搜索"网关如何工作"时，无法确定哪个是最新状态
2. 散落的文件让 AI 缺乏知识入口
3. 日期前缀让文档变成一次性产物，写完就再也不维护
```

### <Good> 规范示例

```markdown
# ✅ 领域主体命名 + 四象限 + 知识索引

规范的四象限组织：
- docs/INDEX.md                        ← 知识导航入口
- docs/specs/gateway-proxy-design.md   ← 随代码演进的长活文档
- docs/plan/gateway-proxy-plan.md      ← 交付后标记 [DONE]
- docs/tracking/gateway-proxy.md       ← 验收后归档/删除
- docs/guide/gateway-proxy-guide.md    ← 面向开发者的 How-to

相对路径链接：
[详细设计](../specs/gateway-proxy-design.md)

INDEX.md 导航：
## 网关代理
| 文档 | 象限 | 文件 |
| :--- | :--- | :--- |
| 设计规范 | specs | [gateway-proxy-design.md](specs/gateway-proxy-design.md) |
| 使用指南 | guide | [gateway-proxy-guide.md](guide/gateway-proxy-guide.md) |
```
