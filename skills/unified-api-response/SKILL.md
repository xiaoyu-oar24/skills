...
不限定具体框架。核心要求是项目实现了统一的 JSON 响应格式封装，常见模式：
...
1. 使用 Glob 或 Grep 查看项目中是否存在已有的响应封装工具（如 `api-response`、`Result`、`ApiResult` 等类/文件）
2. 如果项目已有封装，沿用现有模式；如果尚未建立，则创建统一工具

### 阶段 1：改造处理函数

1. **读取**: 使用 Read 工具读取目标 API Handler 文件
2. **成功响应**: 将裸返回数据重构为统一成功响应格式
...
完成后调用 Skill 工具执行 `self-check-trinity` 验证类型安全和测试通过
